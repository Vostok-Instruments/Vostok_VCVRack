...
class FoldStage1 {
  public:
    ...
    // xt - threshold x
    static float f(float x, float t) { return ((t < x) ? (-m * x + t * (m + 1)) : ((t < -x) ? (-m * x - t * (m + 1)) : (x))); }
...
class FoldStage2 {
  public:
    ...
    static float F(float x) { return (x > 0) ? F_signed(x) : F_signed(-x); }
...
class HardClip {
  ...
  public:
    ...
    static int sgn(float val) { return (0.f < val) - (val < 0.f); }

    // hard clip function
    static float f(float x) { return std::abs(x) > 1.f ? sgn(x) : x; }

    // integral of the hard clip function
    static float F(float x) { return (std::abs(x) > 1.f) ? x * sgn(x) - 0.5f : x * x * 0.5f; }
...
struct Sena : Module {
    ...
    const float defaultFreqCoarse =
        rescale(std::log2(dsp::FREQ_C4), std::log2(kVcoFreqKnobMinCoarse), std::log2(kVcoFreqKnobMaxCoarse), 0.f, 1.f);
    ...
    Sena() {
        ...
        for (int i = 0; i < NUM_CHANNELS; ++i) {
            freqParams[i] = configParam<FreqParamQuantity>(FREQ1_PARAM + i, 0.f, 1.f, defaultFreqCoarse,
                                                           string::f("Ch. %d frequency", i + 1), "Hz");
    ...
    void process(const ProcessArgs &args) override {
        ...
        for (int i = 0; i < oversamplingRatio; ++i) {
            ...
            // floating point arithmetic doesn't work well at low frequencies, specifically because the finite difference
            // denominator becomes tiny - we check for that scenario and use naive / 1st order waveforms in that frequency
            // regime (as aliasing isn't a problem there). With no oversampling, at 44100Hz, the threshold frequency
            // is 44.1Hz.
...
struct SenaWidget : ModuleWidget {
    SenaWidget(Sena *module) {
        ...
        for (int i = 0; i < Sena::NUM_CHANNELS; i++) {
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(41.096, 15.269 + step_y * i)), module, Sena::FREQ1_PARAM + i));
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(58.808, 15.269 + step_y * i)), module, Sena::MOD1_PARAM + i));
    ...
    void appendContextMenu(Menu *menu) override {
        ...
        menu->addChild(createSubmenuItem("Anti-aliasing", "", [=](Menu *menu) {
            menu->addChild(createIndexSubmenuItem(
                "Oversampling", {"Off", "x2", "x4", "x8"}, [=]() { return module->oversamplingIndex; },
                [=](int mode) {
