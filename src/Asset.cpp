...
struct Asset : Module {
    ...
    Asset() {
        ...
        for (int i = 0; i < NUM_CHANNELS; i++) {
            ...
            configSwitch(OFFSET_POLARITY1_PARAM + i, 0.f, 1.f, 0.f, string::f("Ch. %d DC Offset polarity", i + 1),
                         {"Positive", "Negative"});
...
struct AssetWidget : ModuleWidget {
    AssetWidget(Asset *module) {
        ...
        for (int i = 0; i < Asset::NUM_CHANNELS; i++) {
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(27.473, 15.356 + i * step_y)), module, Asset::LEVEL1_PARAM + i));
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(41.971, 15.356 + i * step_y)), module, Asset::OFFSET1_PARAM + i));
            addParam(createParam<CKSSHoriz2>(mm2px(Vec(3.635, 23.511 + i * step_y)), module, Asset::INPUT_POLARITY1_PARAM + i));
            addParam(
                createParam<CKSSHoriz2>(mm2px(Vec(13.577, 23.511 + i * step_y)), module, Asset::OFFSET_POLARITY1_PARAM + i));
            addInput(createInputCentered<PJ301MPort>(mm2px(Vec(6.587, 15.337 + i * step_y)), module, Asset::IN1_INPUT + i));
            addOutput(
                createOutputCentered<PJ301MPort>(mm2px(Vec(15.731, 15.337 + i * step_y)), module, Asset::OUT1_OUTPUT + i));
        }
        ...
        addChild(
            createLight<VostokUpperOrangeNumberLed<6>>(mm2px(Vec(31.941, 111.296)), module, Asset::NUM1_LIGHT + 2 * 5 + 0));
