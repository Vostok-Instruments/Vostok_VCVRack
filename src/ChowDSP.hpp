...
template <int ORDER, typename T = float> struct IIRFilter {
    ...
    void reset() { std::fill(z, &z[ORDER], 0.0f); }
    ...
    template <int N = ORDER> inline typename std::enable_if<N == 2, T>::type process(T x) noexcept {
        T y = z[1] + x * b[0];
        z[1] = x * b[1] - y * a[1];
        return y;
    }

    template <int N = ORDER> inline typename std::enable_if<N == 3, T>::type process(T x) noexcept {
        T y = z[1] + x * b[0];
        z[1] = z[2] + x * b[1] - y * a[1];
        z[2] = x * b[2] - y * a[2];
        return y;
    ...
    */
    ...
    T getFrequencyResponse(T f) { return simd::abs(getTransferFunction(2 * M_PI * f)); }

    T getFrequencyPhase(T f) { return simd::arg(getTransferFunction(2 * M_PI * f)); }
};

template <typename T = float> struct TBiquadFilter : IIRFilter<3, T> {
    enum Type {
        LOWPASS,
        HIGHPASS,
        LOWSHELF,
...
*/
template <int N, typename T> class AAFilter {
  public:
    AAFilter() = default;
 ...
 */
template <typename T> class BaseOversampling {
  public:
...
/**
    ...
    @endcode
*/
template <int ratio, int filtN = 4, typename T = float> class Oversampling : public BaseOversampling<T> {
  public:
    ...
    inline T *getOSBuffer() noexcept override { return osBuffer; }
...
/**
    ...
    @endcode
        ...
        source (modified):
   https://github.com/jatinchowdhury18/ChowDSP-VCV/blob/master/src/shared/VariableOversampling.hpp
*/
template <int filtN = 4, typename T = float> class VariableOversampling {
  public:
    ...
    /** Upsample a single input sample and update the oversampled buffer */
    inline void upsample(T x) noexcept { oss[osIdx]->upsample(x); }

    /** Output a downsampled output sample from the current oversampled buffer */
    inline T downsample() noexcept { return oss[osIdx]->downsample(); }

    /** Returns a pointer to the oversampled buffer */
    inline T *getOSBuffer() noexcept { return oss[osIdx]->getOSBuffer(); }
