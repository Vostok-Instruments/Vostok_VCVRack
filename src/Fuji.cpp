...
struct Fuji : Module {
    ...
    Fuji() {
        ...
        for (int i = 0; i < NUM_CHANNELS; i++) {
            ...
            auto modeSwitch = configSwitch(MODE1_PARAM + i, 0.f, 1.f, 0.f, string::f("Mode Ch. %d", i + 1),
                                           {"Attack Decay Mode", "Hold Mode"});
...
struct FujiWidget : ModuleWidget {
    FujiWidget(Fuji *module) {
        ...
        for (int i = 0; i < Fuji::NUM_CHANNELS; ++i) {
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(27.502, 15.303 + step_y * i)), module, Fuji::ATTACK1_PARAM + i));
            addParam(
                createParamCentered<RoundBlackKnob>(mm2px(Vec(42.002, 15.303 + step_y * i)), module, Fuji::DECAY1_PARAM + i));
