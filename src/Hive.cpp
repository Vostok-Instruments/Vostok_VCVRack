...
struct Hive : Module {
    ...
    void process(const ProcessArgs &args) override {
        ...
        float_4 leftIns = float_4(inputs[LEFT_INPUT + 0].getVoltage(), inputs[LEFT_INPUT + 1].getVoltage(),
                                  inputs[LEFT_INPUT + 2].getVoltage(), inputs[LEFT_INPUT + 3].getVoltage());
        ...
        float_4 panCvIns = float_4(inputs[PAN_INPUT + 0].getVoltage(), inputs[PAN_INPUT + 1].getVoltage(),
                                   inputs[PAN_INPUT + 2].getVoltage(), inputs[PAN_INPUT + 3].getVoltage());
        float_4 panParams = float_4(params[PAN_PARAM + 0].getValue(), params[PAN_PARAM + 1].getValue(),
                                    params[PAN_PARAM + 2].getValue(), params[PAN_PARAM + 3].getValue());
        float_4 pan = simd::clamp(panParams + panCvIns / 2.5f, -1.f, 1.f);

        float_4 gains = float_4(params[GAIN_PARAM + 0].getValue(), params[GAIN_PARAM + 1].getValue(),
                                params[GAIN_PARAM + 2].getValue(), params[GAIN_PARAM + 3].getValue());
...
struct HiveWidget : ModuleWidget {
    HiveWidget(Hive *module) {
        ...
        addChild(createLight<VostokUpperWhiteNumberLed<1>>(mm2px(Vec(31.994, 19.259)), module,
                                                           Hive::NUM_LIGHT + 2 * 0 + 0)); // white top
        addChild(createLight<VostokLowerOrangeNumberLed<1>>(mm2px(Vec(31.994, 19.259)), module,
                                                            Hive::NUM_LIGHT + 2 * 0 + 1)); // orange bottom
