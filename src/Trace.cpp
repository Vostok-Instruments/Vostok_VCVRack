...
struct Trace : Module {
    ...
    void process(const ProcessArgs &args) override {
        ...
        const float scanCv = params[SCAN_CV_PARAM].getValue() * inputs[SCAN_INPUT].getVoltage() / 5.f *
                             (params[PLUS_MINUS_PARAM].getValue() == 0.f ? 1.f : -1.f);
        ...
        float out = inputs[IN1_INPUT].getVoltage() * inGain[0] + inputs[IN2_INPUT].getVoltage() * inGain[1] +
                    inputs[IN3_INPUT].getVoltage() * inGain[2] + inputs[IN4_INPUT].getVoltage() * inGain[3];
